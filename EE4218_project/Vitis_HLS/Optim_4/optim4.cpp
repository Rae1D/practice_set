...
void hidden_layer(
    ...
    int bias[2])
{
    // Include the activation inside the hidden layer to save an array being passed  
    ...
    for (int t = 0; t < 64; t++) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            // do the activation
            output[t][i] = sigmoid_lookup[output[t][i]]; // Use the lookup table for sigmoid
        }
    }
}
...
void optim4(hls::stream<AXIS_wLAST>& S_AXIS, hls::stream<AXIS_wLAST>& M_AXIS)
{
    ...
    #pragma HLS ARRAY_PARTITION variable=output_hidden complete dim=1
    #pragma HLS ARRAY_PARTITION variable=output_hidden cyclic dim=0 factor=4
    // int output_sigmoid[ROWS][2]; // stores 8-bit unsigned values (0-255)    
    int pred[ROWS] = {0};

    int inputs[ROWS][COLS]; // stores 8-bit unsigned values (0-255)
    // #pragma HLS ARRAY_PARTITION variable=inputs complete dim=1 // Partition the COLS for parallel access
    int bias_hidden[2];
    #pragma HLS ARRAY_PARTITION variable=bias_hidden complete dim=0
    int weight_hidden[7][2];
    // #pragma HLS ARRAY_PARTITION variable=weight_hidden complete dim=0 // Partition the weight array for parallel access
    int bias_output[1];
    ...
    for (int i = 0; i < 2; i++) {
        // Read each value, expecting an integer followed by a comma or end of line
        read_input = S_AXIS.read();
        weight_output[i][0] = read_input.data;
    }

    // BEGIN MLP OPERATIONS -------------------------------------------------------------------------
    hidden_layer(inputs, output_hidden, weight_hidden, bias_hidden);
    output_layer(output_hidden, pred, weight_output, bias_output);
